...
Does NOT attempt raw InSAR raster (GeoTIFF) ingestion or the raster-
specific quadtree/uniform PIXEL downsampling that belongs to
core.insar_raster_import (which operates on regular lon2d/lat2d grid
arrays) -- that stays upstream, in whatever InSAR processing chain
(LiCSBAS, MintPy, GMTSAR, ...) produced the point/vector product.

This module DOES provide a separate, general-purpose downsampling pair
(downsample_rows_uniform_stride / downsample_rows_grid) for arbitrary
SCATTERED point rows (already-built {"lon","lat",...} dicts, from
EITHER schema) -- e.g. a user's own ENU-displacement point grid sampled
in QGIS off some other raster/product this plugin has no reader for,
then exported/loaded as a CSV or point layer through the normal "gnss"
path here. That case has no regular lon2d/lat2d array to hand
core.insar_raster_import's functions, so a scattered-point-aware
grid-bin decimation is needed instead. See _run_slip_inversion's/
_greens_unit_matrices_mp's own docstrings in dc3d_worker.py for why
observation COUNT (not just patch count) directly drives Green's-matrix
assembly cost -- thinning here is the cheapest lever available before
resorting to worker-side multiprocessing.
"""
...
# ─── Scattered-point downsampling (for already-built observation rows) ────
#
# General-purpose thinning for lists of {"lon","lat",...} dicts, usable
# on the output of build_observations_from_mapped_rows() for EITHER
# schema (gnss or insar_los) -- unlike core.insar_raster_import's
# downsample_uniform()/downsample_quadtree(), which require a regular
# lon2d/lat2d raster array and therefore cannot be reused here.

def downsample_rows_uniform_stride(rows: List[dict], stride: int) -> List[dict]:
    """Keep every `stride`-th row, in original (file/layer) order.
    Simplest possible thinning -- no spatial awareness at all, so it can
    leave gaps or clusters if the input rows aren't already roughly
    evenly ordered/spaced. stride < 2 returns rows unchanged."""
...
def downsample_rows_grid(rows: List[dict], max_points: int,
                         lon_key: str = "lon", lat_key: str = "lat") -> List[dict]:
    """
    Grid-bin decimation for scattered point rows: divides the point
    cloud's bounding box into a roughly-square grid of cells sized to
    yield approximately `max_points` occupied cells (correcting for
    latitude-dependent longitude spacing via cos(mean latitude)), then
    keeps ONE row per occupied cell -- specifically the row whose
    (lon,lat) is nearest that cell's centroid, so the retained point is
    spatially representative rather than an accident of file order (as
    a plain positional stride would give).

    Unlike core.insar_raster_import.downsample_uniform()/
    downsample_quadtree() (which require a regular lon2d/lat2d raster
    array), this works on ANY list of dicts with lon/lat keys --
    including points a user already sampled onto a point grid/layer in
    QGIS from a product this plugin has no dedicated raster reader for
    (e.g. pre-resolved E/N/U rasters fed through a QGIS point-sampling
    tool rather than through core.insar_raster_import).

    Returns rows unchanged if len(rows) <= max_points or max_points <= 0
    (nothing to do). Never raises on degenerate input (all points at
    one location, a single row, etc.) -- falls back to returning rows
    as-is in those cases since there is no meaningful grid to bin into.
    """
    n = len(rows)
    if n <= max_points or max_points <= 0:
        return list(rows)

    lons = [float(r[lon_key]) for r in rows]
    lats = [float(r[lat_key]) for r in rows]
    lon_min, lon_max = min(lons), max(lons)
    lat_min, lat_max = min(lats), max(lats)
    lat0 = sum(lats) / n
    coslat = max(math.cos(math.radians(lat0)), 1e-6)

    # Common physical unit (degrees, longitude compressed by cos(lat))
    # so cell sizing isn't distorted by latitude.
    ...
    if x_span <= 1e-9 or y_span <= 1e-9:
        # Degenerate (all points on one line/point) -- no meaningful
        # 2-D grid to bin into; fall back to a uniform positional stride
        # so at least SOME thinning still happens.
        return downsample_rows_uniform_stride(rows, math.ceil(n / max_points))

    cell = math.sqrt((x_span * y_span) / max_points)
    n_cols = max(1, math.ceil(x_span / cell))
    n_rows_grid = max(1, math.ceil(y_span / cell))

    buckets: Dict[Tuple[int, int], List[int]] = {}
    for idx in range(n):
        col = min(n_cols - 1, int((xs[idx] - x_min) / cell))
        row_i = min(n_rows_grid - 1, int((ys[idx] - y_min) / cell))
        buckets.setdefault((row_i, col), []).append(idx)

    kept_idx = []
    for (row_i, col), idxs in buckets.items():
        if len(idxs) == 1:
            kept_idx.append(idxs[0])
            continue
        cx = x_min + (col + 0.5) * cell
        cy = y_min + (row_i + 0.5) * cell
        best_idx = min(idxs, key=lambda i: (xs[i] - cx) ** 2 + (ys[i] - cy) ** 2)
