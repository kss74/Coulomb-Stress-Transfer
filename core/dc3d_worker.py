...
def _run_slip_inversion(dc3dwrapper, job):
    ...
    L_full = _laplacian_full(np, [(n_length, n_width)])

    return _solve_slip_inversion(
        dc3dwrapper, np, patches, L_full, observations, los_observations,
        mu, nu, smoothing_factor, max_slip, target_mw)
...
def _run_slip_inversion_group(dc3dwrapper, job):
    ...
    L_full = _laplacian_full(np, shapes)
...
def _laplacian_full(np, shapes):
    """
    Block-diagonal Laplacian roughness matrix (2*n_p_total columns, the
    two slip channels stacked as in _solve_slip_inversion()) across one
    or more (n_length, n_width) fault segments, each smoothed only
    against its OWN along-strike/down-dip neighbors (same indexing as
    core.okada_engine.FaultParameters.subdivide(): i=down-dip,
    j=along-strike, flat i*n_length+j) -- no cross-segment terms, and a
    single (n_length, n_width) entry reproduces exactly the roughness
    matrix _run_slip_inversion() built inline before this was factored
    out.
    """
    ...
    L_full = (np.block([[L_base, zeros_block], [zeros_block, L_base]])
              if L_base.shape[0] > 0 else np.zeros((0, 2 * n_p_total)))
    return L_full


def _solve_slip_inversion(dc3dwrapper, np, patches, L_full, observations,
    ...
    subdivision or several concatenated fault segments. `L_full` is
    already fully built (see _laplacian_full()) by the caller.
    """
    import math
    ...
    if observations:
        Ge_rt, Gn_rt, Gu_rt, Ge_rev, Gn_rev, Gu_rev = _greens_unit_matrices(
            dc3dwrapper, np, patches, observations, mu, nu)
    if los_observations:
        Ge_rt_l, Gn_rt_l, Gu_rt_l, Ge_rev_l, Gn_rev_l, Gu_rev_l = _greens_unit_matrices(
    ...
    if target_mw is None:
        ...
        if L_full.shape[0] > 0:
            A = np.vstack([G, smoothing_factor * L_full])
            b = np.concatenate([d, np.zeros(L_full.shape[0])])
        else:
            A, b = G, d
        ...
        #      moment mu*sum(area_p * hypot(rt_p, rev_p)) is now
        #      NONLINEAR in the two independent per-patch unknowns, so
        #      this uses a NonlinearConstraint with an analytic
        #      Jacobian rather than the original's LinearConstraint.
        ...
        eps = 1e-6  # keeps hypot's gradient finite at exactly zero slip
        ...
        def objective(x):
            residual = G @ x - d
            roughness = L_full @ x if L_full.shape[0] > 0 else np.zeros(0)
            return float(residual @ residual + (smoothing_factor**2) * (roughness @ roughness))

        GTG, GTd = G.T @ G, G.T @ d
        LTL = (L_full.T @ L_full) if L_full.shape[0] > 0 else np.zeros((2 * n_p, 2 * n_p))
    ...
    final_moment = mu * float(np.sum(areas_m2 * np.sqrt(x[:n_p]**2 + x[n_p:]**2)))
    ...
    return {
        ...
        "n_iter": n_iter,
        "slip": [[float(x[p]), float(x[n_p + p])] for p in range(n_p)],
        "rms_misfit": rms,
