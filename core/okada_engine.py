...
def _run_dc3d_worker_points_full(sources, elastic, points):
    """
    Run the external DC3D worker in "points_full" mode (2026-09-01,
    core.point_calculation "point calculator" addition): same
    points-with-own-depth schema as
    _run_dc3d_worker_cross_section_stress_tensor(), but returns BOTH the
    raw stress tensor and the displacement vector per point in a single
    subprocess round-trip -- compute_point_results() always needs both
    quantities at the same points, so combining them avoids launching
    the external Python twice for the same source/point geometry loop.

    Returns (sxx_pa, syy_pa, szz_pa, sxy_pa, sxz_pa, syz_pa,
    ue_m, un_m, uz_m), each a flat numpy array with one value per input
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        return (np.array(payload["sxx_pa"]), np.array(payload["syy_pa"]),
               np.array(payload["szz_pa"]), np.array(payload["sxy_pa"]),
               np.array(payload["sxz_pa"]), np.array(payload["syz_pa"]),
               np.array(payload["ue_m"]), np.array(payload["un_m"]), np.array(payload["uz_m"]))


def resolve_cff_shear_normal(stress: dict, receiver: FaultParameters, friction: float):
    """
    Resolve a stress tensor dict (sxx..syz, geographic E/N/Down frame,
    Pa, tension-positive) onto `receiver`'s strike/dip/rake plane,
    returning (cff, shear, normal) as a 3-tuple of arrays -- the same
    ΔCFF = Δτ + μ'·Δσn (King et al. 1994) as compute_cff() above, except
    compute_cff() only returns the combined value, and
    core.point_calculation.compute_point_results() needs shear/normal
    reported separately too (same as the per-fault reporting
    compute_cff_on_receiver_faults() already does inline).

    Deliberately duplicates the ~10-line normal/slip-vector geometry
    from compute_cff() rather than refactoring compute_cff() itself to
    return three values -- consistent with this project's existing
    pattern of repeating this snippet at each call site that needs it
    (dc3d_worker.py's "cff"/"cross_section" modes,
    compute_cff_on_receiver_faults() above) rather than touching an
    already-validated shared function and every one of its callers.
    """
    sr = np.deg2rad(receiver.strike); dr = np.deg2rad(receiver.dip); rr = np.deg2rad(receiver.rake)
    cs, ss = np.cos(sr), np.sin(sr); cd, sd = np.cos(dr), np.sin(dr); cr, sr_ = np.cos(rr), np.sin(rr)
    nx, ny, nz = cs * sd, -ss * sd, -cd
    ...
    ty = sxy * nx + syy * ny + syz * nz
    tz = sxz * nx + syz * ny + szz * nz
    shear = tx * lx + ty * ly + tz * lz
    normal = tx * nx + ty * ny + tz * nz
    cff = shear + friction * normal
    return cff, shear, normal
