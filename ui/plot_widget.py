...
class PlotWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._slip_validation_axes = []  # tracks the extra axes created
                                # on demand by plot_slip_inversion_validation()
                                # (three side-by-side imshow panels + a
                                # second colorbar) -- same "separate
                                # objects, self.ax/self.cax hidden not
                                # reused" pattern as _stereonet_ax above,
                                # just a list since there are more than
                                # one. See _reset_axes().
    ...
    def _reset_axes(self):
        ...
        # Same "remove, don't just clear" reasoning for any extra axes
        # left over from a PREVIOUS plot_slip_inversion_validation()
        # call -- three side-by-side panels plus a second colorbar are
        # all separate Axes objects, not sub-regions of self.ax/self.cax.
        for ax in self._slip_validation_axes:
            ax.remove()
        self._slip_validation_axes = []
    ...
    def plot_slip_inversion_result(self, mag_grid, rms_misfit=None, achieved_mw=None,
                                   fixed_rake_deg=None, title_extra=None):
        """
        The MAIN, always-shown output of a slip inversion: a single
        patch-grid heatmap of the inverted slip magnitude, in the
        plugin's normal single self.ax/self.cax layout (same pattern as
        plot_cff()/plot_displacement() above) -- unlike
        plot_slip_inversion_validation() below, which needs a reference
        model and three panels and is entirely OPTIONAL/secondary
        (2026-08-30 redesign: showing the inverted slip itself needs no
        reference model at all, so it shouldn't be gated behind one).

        mag_grid       : (n_width, n_length) array of slip magnitude
                         (m) -- e.g.
                         np.hypot(rt, rev).reshape(n_width, n_length)
                         from a single fault segment's own slice of
                         diag["slip"] (or, for a fixed-rake run, the
                         signed magnitude along that rake).
        rms_misfit,
        achieved_mw,
        fixed_rake_deg : optional solver diagnostics, shown as a small
                         caption line if given (same values
                         SlipInversionDialog's own result_label already
                         displays as text -- this just repeats them
                         next to the plot for convenience).
        title_extra    : e.g. the fault's own name, or the currently
                         selected segment's name for a Group run.
        """
        self._reset_axes()

        vmax = max(float(np.nanmax(mag_grid)), 1e-6)
        im = self.ax.imshow(mag_grid, cmap="viridis", vmin=0, vmax=vmax, aspect="auto")
        self.figure.colorbar(im, cax=self.cax, label="slip (m)")

        title = "Inverted slip magnitude"
        if title_extra:
            title += f"  [{title_extra}]"
        self.ax.set_title(title, fontsize=10, fontweight="bold")
        self.ax.set_xlabel("along-strike patch j", fontsize=8)
        self.ax.set_ylabel("down-dip patch i", fontsize=8)

        caption_bits = []
        if rms_misfit is not None:
            caption_bits.append(f"RMS misfit={rms_misfit:.4g}")
        if achieved_mw is not None:
            caption_bits.append(f"achieved Mw={achieved_mw:.2f}")
        if fixed_rake_deg is not None:
            caption_bits.append(f"fixed rake={fixed_rake_deg:.1f}\u00b0")
        if caption_bits:
            self.ax.text(0.0, -0.20, "   |   ".join(caption_bits),
                         transform=self.ax.transAxes, fontsize=7.5, color="dimgray")
        self.canvas.draw()

    def plot_slip_inversion_validation(self, result, title_extra=None):
        """
        Three side-by-side patch-grid maps comparing an inverted slip
        distribution against an independent reference model:
        reference slip magnitude | inverted slip magnitude | difference
        -- plus a text box of the summary statistics
        (core.slip_inversion_validation.SlipValidationResult).

        `result` : a core.slip_inversion_validation.SlipValidationResult.

        Same "hide self.ax/self.cax, add fresh axes at their figure-
        fraction footprint" pattern as plot_stress_inversion_stereonet()
        above, generalized to three panels (tracked in
        self._slip_validation_axes, removed in _reset_axes()) since a
        1x2-axes fixed layout can't be repurposed into 1x3 + an extra
        colorbar in place.
        """
        self._reset_axes()
        self.ax.set_visible(False)
        ...
        mag_true = result.mag_true_grid()
        mag_inv = result.mag_inv_grid()
        diff = mag_inv - mag_true
        vmax = max(float(np.nanmax(mag_true)), float(np.nanmax(mag_inv)), 1e-6)
        dmax = max(float(np.nanmax(np.abs(diff))), 1e-6)

        # Three panels + a shared slip colorbar + a separate diff
        # colorbar, laid out left-to-right across the same overall
        # footprint self.ax/self.cax normally occupy. Only the leftmost
        # panel keeps y-tick labels (down-dip index) -- repeating them
        # on all three is redundant clutter and, combined with the
        # colorbar sitting between panels, was genuinely hard to read
        # in testing.
        left0, bottom, panel_w, height = 0.05, 0.16, 0.22, 0.68
        gap = 0.012
        ax0 = self.figure.add_axes([left0, bottom, panel_w, height])
        ax1 = self.figure.add_axes([left0 + panel_w + gap, bottom, panel_w, height])
        cax_slip_x = left0 + 2 * (panel_w + gap)
        cax_slip = self.figure.add_axes([cax_slip_x, bottom, 0.015, height])
        ax2_x = cax_slip_x + 0.015 + 0.05
        ax2 = self.figure.add_axes([ax2_x, bottom, panel_w, height])
        cax_diff = self.figure.add_axes([ax2_x + panel_w + gap, bottom, 0.015, height])
        self._slip_validation_axes = [ax0, ax1, cax_slip, ax2, cax_diff]

        im0 = ax0.imshow(mag_true, cmap="viridis", vmin=0, vmax=vmax, aspect="auto")
        ax0.set_title("Reference", fontsize=9)
        ax1.imshow(mag_inv, cmap="viridis", vmin=0, vmax=vmax, aspect="auto")
        ax1.set_title("Inverted", fontsize=9)
        ax1.set_yticklabels([])
        self.figure.colorbar(im0, cax=cax_slip, label="slip (m)")

        im2 = ax2.imshow(diff, cmap="RdBu_r", vmin=-dmax, vmax=dmax, aspect="auto")
        ax2.set_title("Inverted \u2212 reference", fontsize=9)
        ax2.set_yticklabels([])
        self.figure.colorbar(im2, cax=cax_diff, label="diff (m)")

        for ax in (ax0, ax1, ax2):
            ax.set_xlabel("along-strike j", fontsize=7.5)
            ax.tick_params(labelsize=7)
        ax0.set_ylabel("down-dip i", fontsize=7.5)

        corr_text = ("corr = n/a" if result.magnitude_corr != result.magnitude_corr  # NaN check
                    else f"corr = {result.magnitude_corr:.3f}")
        stats_text = (
            f"{corr_text}   RMS = {result.magnitude_rms_m:.3f} m   "
            f"true Mw = {result.true_mw:.2f}   achieved Mw = {result.achieved_mw:.2f}"
        )
        self.figure.text(0.5, 0.03, stats_text, ha="center", va="bottom",
                         fontsize=8, family="monospace",
                         bbox=dict(boxstyle="round", fc="white", ec="0.6", alpha=0.9))

        title = "Slip inversion validation against reference model"
        if title_extra:
            title += f"  [{title_extra}]"
        self.figure.suptitle(title, fontsize=10, fontweight="bold", y=0.99)
        self.canvas.draw()

    def save_to_file(self, path, dpi=300):
        """
        Save the currently-displayed plot (CFF map, displacement map,
        cross-section, stereonet, or slip-inversion validation —
        whichever was drawn most recently) to an image file. Format is
        inferred from the file extension; matplotlib
